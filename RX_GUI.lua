...
local Paragraph = InfoTab:CreateParagraph({Title = "Hotbar Editor", Content = "Be Able To Rename Your Hotbar."})
...
local Paragraph = InfoTab:CreateParagraph({Title = "Counters", Content = "Tools That Play Certain Characters Counter Animations."})
...
-- SCRIPT DISABLER
local scriptEnabled = true -- Set this to true to enable the script

-- Toggle function to enable or disable the script
local function toggleScript(enabled)
    scriptEnabled = enabled
    if not enabled then
        warn("Script has been disabled.")
        script.Parent = nil -- Destroy the script
    else
        warn("Script has been enabled.")
    end
end
-- END
-- PART CREATION
...
-- END
-- ANIMATION DETECTION [CRUSHING PULL GROUND VAR × > LETHAL WHIRLWIND ] ✓
function onAnimation(id, func)
    local id = tostring(id):gsub("rbxassetid://", "")
...
-- END
-- ANIMATION DETECTION [ATMOS CLEAVE] ✓
function onAnimation(id, func)
    local id = tostring(id):gsub("rbxassetid://", "")
...
-- END
-- ANIMATION DETECTION [FLOWING WATER] ✓
function onAnimation(id, func)
    local id = tostring(id):gsub("rbxassetid://", "")
...
-- END
-- ANIMATION DETECTION [CRUSHING PULL AIR VAR]
function onAnimation(id, func)
    local id = tostring(id):gsub("rbxassetid://", "")
...
onAnimation("18717031831", function(animation)
    wait(0.8)
    teleportPlayer()
    warn("animation id:", animation.Animation.AnimationId)
end)
...
local Input = TpTab:CreateInput({
   ...
   RemoveTextAfterFocusLost = false,
   Callback = function(Text)
game.Players.LocalPlayer.Character:SetPrimaryPartCFrame(CFrame.new((Text)))
   end,
})
